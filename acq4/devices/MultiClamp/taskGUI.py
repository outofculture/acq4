...
class MultiClampTaskGui(TaskGui):
    ...
    def quit(self):
        ...
        if not sip.isdeleted(self.daqUI):
           self.daqUI.sigChanged.disconnect(self.daqChanged)  #Ronny: Fix for error when loading protocols in taskrunner (Qt.SIGNAL('changed') to Qt.QtCore.SIGNAL('changed') )
